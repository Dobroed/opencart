...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1'  AND p.stock_status_id='7' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
	...
	public function getTotalProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "'   AND p.stock_status_id='7' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
	...
	public function getTotalProductSpecials() {
       ...
       public function getProductsInSubGroups($data = array()) {
			...
			if (!empty($data['filter_sub_category'])) {
				...
				//$sql .= "  AND (p2c.category_id = '" . (int)$data['filter_category_id'] . "')";
                            $sql .= "  AND (p2c.category_id IN ("; 
                             foreach ($data['filter_category_id'] as $subcategory) {
                                  if (!next($data['filter_category_id'])) {
                                     $sql.=$subcategory['category_id'];
                                     } else { $sql.=$subcategory['category_id'].","; }
                             }
                            $sql .="))";			
        ...
        public function getTotalProductsInSubGroups($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
